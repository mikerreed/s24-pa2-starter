...
static void draw_corner(GCanvas* canvas, const GColor& c, float x, float y, float dx, float dy) {
    canvas->drawRect(make_from_pts({x, y - 1}, {x + dx, y + 1}), GPaint(c));
    canvas->drawRect(make_from_pts({x - 1, y}, {x + 1, y + dy}), GPaint(c));
}
        ...
        for (int i = fList.size() - 1; i >= 0; --i) {
            if (contains(fList[i]->getRect(), loc.x, loc.y)) {
                ...
                return new GClick(loc, [this](GClick* click) {
                    ...
                    fShape->setRect(fShape->getRect().offset(curr.x - prev.x, curr.y - prev.y));
                    this->updateTitle();
                    this->requestDraw();
                });
            }
